package itba.edu.ar.pfexpobeaconbe.api.stand.application;

import itba.edu.ar.pfexpobeaconbe.api.stand.domain.Stand;
import itba.edu.ar.pfexpobeaconbe.api.stand.domain.StandRepository;
...
public class StandService {
    ...
    public List<Stand> listOrderedByRanking() {
        return standRepository.findByOrderByRankingAverage_RankingDesc();
    }

    public void updateAverage(final Map<String, Double> standAverage) {
        final List<Stand> stands = standRepository.findAllById(standAverage.keySet());
        stands.forEach(stand -> stand.setAverageTime(standAverage.get(stand.getId())));
